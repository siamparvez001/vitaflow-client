    ...
    useEffect(() => {
        if (user) {
            setProfileData({
                ...
                bloodGroup: user?.data?.bloodGroup || "O+",
                district: user?.data?.district || "dhaka",
                upazila: user?.data?.upazila || "dhanmondi",
                title: user?.data?.title || "Clinical Administrator • Central Region Hospital",
                donorStatus: user?.data?.donorStatus || "Active Donor",
                visibility: user?.data?.visibility !== false
            });
        }
    }, [user]);
